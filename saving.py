...
class SaveHandler(Handler):
    ...
    def loadProject(self, info):
        ...
        if fileDialog.path == '' or fileDialog.return_code == CANCEL:
            ...
            extLen = len(self.extension)
            if extLen and fileDialog.path[-extLen - 1:] != '.' + self.extension:
                error(None, 'Project file must have a %s extension' %self.extension, title='Wrong file extension')
                return
            try:
                self.saveObject.filepath = fileDialog.path
...
class BaseSaveHandler(SaveHandler):
    def rem_selected(self, info):
        info.object._rem_selected()

    def import_selected(self, info):
        info.object._import_selected()

    def add_new(self, info):
        info.object._add_new()
